...
## Дві мови (RU основна, UA друга)

Зараз це реалізовано як два дзеркальні статичні файли:
- `index.html` — російська (за замовчуванням)
- `ua/index.html` — українська, з посиланнями на спільні
  `../style.css` і `../assets/*`

У WordPress цій структурі відповідає плагін **WPML** або **Polylang**:
кожна мова — окрема сторінка/переклад, а не JS-перемикач на льоту.
При перенесенні в Elementor:
1. Створіть сторінку "VIP Tattoo School" (RU) — основна мова сайту.
2. Через WPML/Polylang додайте переклад-дублікат тієї ж сторінки
   для UA, перенесіть туди текст з `ua/index.html`.
3. Перемикач мов (`.lang-switch` у хедері) замініть на нативний
   Language Switcher widget від WPML/Polylang — він сам підставить
   посилання на відповідні мовні версії сторінки.
